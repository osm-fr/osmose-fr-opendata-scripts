...
do
  ...
  PG_USE_COPY=YES ogr2ogr -f pgdump /vsistdout/ /vsigzip/pci-$DEP-zoncommuni.json.gz -nlt geometry -nln cadastre_voies -t_srs EPSG:3857 -lco CREATE_TABLE=OFF | ${PSQL} "${DB_BASE}"
done
